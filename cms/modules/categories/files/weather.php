...
if(isset($_POST['submit'])){
	...
	foreach($_months as $m => $month){
		if($_POST['min_temp'][$m-1] != "" && $_POST['max_temp'][$m-1] != ""){
			$exists = db_row('SELECT * FROM category_weather WHERE id_category = ? AND month = ?', intval($_GET['id']), $m);
			if($exists){
				db_query('UPDATE category_weather SET icon = ?, min_temp = ?, max_temp = ? WHERE month = ? AND id_category = ?', 
					$_POST['icon'][$m-1], 
					$_POST['min_temp'][$m-1],
					$_POST['max_temp'][$m-1],
					$m, 
				...
				db_query('INSERT INTO category_weather SET icon = ?, month = ?, min_temp = ?, max_temp = ?, id_category = ?', 
					$_POST['icon'][$m-1], 
					$m, 
					$_POST['min_temp'][$m-1],
					$_POST['max_temp'][$m-1],
	...
	go_away($_base_cms."modules/categories/files/weather.php?id=".intval($_GET['id'])."&edited=1");
}
...
$item = db_row('SELECT * FROM category WHERE id_category = ?', intval($_GET['id']));

$rows = db_query('SELECT * FROM category_weather WHERE id_category = ? ORDER BY `month` ASC', intval($_GET['id']));
foreach($rows as $row){
	$_data[$row['month']] = $row;
}
