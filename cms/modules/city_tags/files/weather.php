...
if(isset($_POST['submit'])){
	...
	foreach($_months as $m => $month){
		if($_POST['min_temp'][$m-1] != "" && $_POST['max_temp'][$m-1] != ""){
			$exists = db_row('SELECT * FROM city_tag_weather WHERE id_city_tag = ? AND month = ?', intval($_GET['id']), $m);
			if($exists){
				db_query('UPDATE city_tag_weather SET icon = ?, min_temp = ?, max_temp = ? WHERE month = ? AND id_city_tag = ?', 
					$_POST['icon'][$m-1], 
					$_POST['min_temp'][$m-1],
					$_POST['max_temp'][$m-1],
					$m, 
				...
				db_query('INSERT INTO city_tag_weather SET icon = ?, month = ?, min_temp = ?, max_temp = ?, id_city_tag = ?', 
					$_POST['icon'][$m-1], 
					$m, 
					$_POST['min_temp'][$m-1],
					$_POST['max_temp'][$m-1],
	...
	go_away($_base_cms."modules/city_tags/files/weather.php?id=".intval($_GET['id'])."&edited=1");
}
...
$item = db_row('SELECT * FROM city_tag WHERE id_city_tag = ?', intval($_GET['id']));

$rows = db_query('SELECT * FROM city_tag_weather WHERE id_city_tag = ? ORDER BY `month` ASC', intval($_GET['id']));
foreach($rows as $row){
	$_data[$row['month']] = $row;
}
