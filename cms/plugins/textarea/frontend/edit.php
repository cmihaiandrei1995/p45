<? $i=1; foreach($field['lng'] as $lng){?>
		...
		<div class="col-md-8 col-lg-10">
			<? if($field['do_not_edit']){?>
				<div class="table-bordered" style="padding:10px">
					<?=$_record[$lng][$field['db_name']]?>
				</div>
			<? }else{ ?>
				<textarea name="<?=$field['id'].'_'.$lng?>" rows="10" class="form-control" id="<?=$field['id'].'_'.$lng?>"><?=(isset($_POST[$field['id'].'_'.$lng]) ? $_POST[$field['id'].'_'.$lng] : $_record[$lng][$field['db_name']])?></textarea>
	...
	<? if($field['use_wysiwyg'] && !$field['do_not_edit']){?>
