<? $i=1; foreach($field['lng'] as $lng){?>
	...
	if(isset($_GET['duplicate'])){
		...
		if(isset($_POST[$field['id'].'_'.$lng.'_id'])) {
			...
			$image_id = $_record[$lng][$_section['id']];
		}
		...
		if(isset($_POST[$field['id'].'_'.$lng.'_id'])) {
			...
			foreach($_record[$lng][$field['use_other_table']] as $j => $val){
				$images[$lng][$val['order']]['folder'] = $val['folder'];
				$images[$lng][$val['order']]['image'] = $val['image'];
				$images[$lng][$val['order']]['copyright'] = $val['copyright'];
				$images[$lng][$val['order']]['title'] = $val['title'];
			...
			foreach($_record[$lng][$field['use_other_table']] as $j => $val){
				...
				$ext_break = explode(".", $val['image']);
				$ext = $ext_break[count($ext_break)-1];
				$code = activation_code(4);

				$tmp_file = explode("-", substr($val['image'], 0, (-1)*strlen($ext)-1));
				unset($tmp_file[count($tmp_file)-1]);

				$filename_new = strtolower(implode("-", $tmp_file)."-".$code.".".$ext);

				if(count($field['sizes'])){
					foreach($field['sizes'] as $key => $size) {
						@copy($_base_uploads_path.'images/'.$val['folder'].$key.'-'.$val['image'], $dirname.$key.'-'.$filename_new);
					}
					@copy($_base_uploads_path.'images/'.$val['folder'].$val['image'], $dirname.$filename_new);
				}
				...
				$id_img = db_query($_insert, $image_id, $val['order'], date('Y').'/'.date('n').'/'.date('j').'/', $filename_new);

				$images[$lng][$val['order']]['folder'] = date('Y').'/'.date('n').'/'.date('j').'/';
				$images[$lng][$val['order']]['image'] = $filename_new;
				$images[$lng][$val['order']]['copyright'] = $val['copyright'];
				$images[$lng][$val['order']]['title'] = $val['title'];
				$images[$lng][$val['order']]['id'] = $id_img;
		...
		foreach($_record[$lng][$field['use_other_table']] as $val){
			...
			$imgs_ids[] = $val['id_image'];
		}

	}

	include($_base_cms_path.'plugins/multi_image_copy_title/popup/delete.php');
	?>
		...
		<div class="col-md-6 col-lg-10">
				...
				<ul>
					<? for($j=1; $j<=count($images[$lng]); $j++){?>
						...
						if(is_url($images[$lng][$j]['image'])){
							...
							?>
							<li data-id="<?=$images[$lng][$j]['id']?>" style="height:350px;">
								...
								<div class="actions">
									...
									--><a href="#delete-image-<?=$field['id'].'-'.$lng?>" class="mb-xs mt-xs mr-xs btn btn-default delete-image-<?=$field['id'].'-'.$lng?>" data-id="<?=$images[$lng][$j]['id']?>" data-table="<?=$_section['table']?>" data-section="<?=$_module?>" data-field="<?=$field_id?>" data-toggle="tooltip" data-placement="top" data-original-title="<?=_lng('delete')?>"><i class="fa fa-trash-o"></i></a>
									<div class="info">
								...
								<div class="text-left">
									<label for="copyright_<?=$lng?>_<?=$images[$lng][$j]['id']?>" class="control-label">Copyright</label>
									<input type="text" class="form-control" id="copyright_<?=$lng?>_<?=$images[$lng][$j]['id']?>" name="copyright_<?=$lng?>_<?=$images[$lng][$j]['id']?>" value="<?=(isset($_POST['copyright_'.$lng.'_'.$images[$lng][$j]['id']]) ? $_POST['copyright_'.$lng.'_'.$images[$lng][$j]['id']] : $images[$lng][$j]['copyright'])?>">
									<label for="title_<?=$lng?>_<?=$images[$lng][$j]['id']?>" class="control-label"><?=_lng('title')?></label>
									<input type="text" class="form-control" id="title_<?=$lng?>_<?=$images[$lng][$j]['id']?>" name="title_<?=$lng?>_<?=$images[$lng][$j]['id']?>" value="<?=(isset($_POST['title_'.$lng.'_'.$images[$lng][$j]['id']]) ? $_POST['title_'.$lng.'_'.$images[$lng][$j]['id']] : $images[$lng][$j]['title'])?>">
								</div>
							</li>
			...
			<? if(!$field['do_not_edit']){?>
				<input
		    		...
		    		value="<?=(isset($_POST[$field['id'].'_'.$lng.'_order']) ? $_POST[$field['id'].'_'.$lng.'_order'] : implode(",", $imgs_ids))?>"
				/>
				<input
					type="hidden"
				...
				<script type="text/javascript">
					$(document).ready(function(){
						...
						$('#<?=$field['id'].'_'.$lng?>').on('fileuploaded', function(event, data, previewId, index) {
				        	...
				        	$('#gal-<?=$field['id'].'_'.$lng?>').removeClass('ui_hidden');
				        	$('#gal-<?=$field['id'].'_'.$lng?> ul').append(
				        		'<li data-id="' + response.id_img + '" style="height:350px;">' +
									...
									'</div>' +
									'<div class="text-left">' +
										'<label for="copyright_<?=$lng?>_' + response.id_img + '" class="control-label">Copyright</label>' +
										'<input type="text" class="form-control" id="copyright_<?=$lng?>_' + response.id_img + '" name="copyright_<?=$lng?>_' + response.id_img + '" value="">' +
										'<label for="title_<?=$lng?>_' + response.id_img + '" class="control-label"><?=_lng('title')?></label>' +
										'<input type="text" class="form-control" id="title_<?=$lng?>_' + response.id_img + '" name="title_<?=$lng?>_' + response.id_img + '" value="">' +
									'</div>' +
								'</li>'
				        	);
				        	reorder_images();
