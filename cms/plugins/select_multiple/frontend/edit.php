<? $i=1; foreach($field['lng'] as $lng){?>
	<div id="field_<?=$field_id.'_'.$lng?>"
		...
		<? $recs = explode(",", $_record[$lng][$field['db_name']]); ?>
		...
		<div class="col-md-6 col-lg-10">
			<select
				...
				data-placeholder="<?=($field['placeholder']!="" ? $field['placeholder'] : "ex: Alege")?>"
				multiple
				<? if($field['do_not_edit']){?>disabled<? }?>>
					<? foreach($field['values'] as $key => $val){?>
			        	<option value="<?=$key?>" <? if(isset($_POST[$field['id'].'_'.$lng]) && in_array($key, $_POST[$field['id'].'_'.$lng])) echo "selected"; elseif(in_array($key, $recs)) echo "selected"?>><?=$val?></option>
			        <? }?>
		    </select>
