	...
	<div id="field_<?=$field_id.'_'.$lng?>"
		...
		<div class="col-md-6 col-lg-10 ">
	    	...
	    	<? if($field['use_ajax']){?>
				...
				$records = array();
	        	foreach($_record[$lng][$field['use_other_table']] as $key => $val){
	        		$records[] = $val[$field['db_name']];
	        	}
	        	?>

	    		<input
	    			type="hidden"
	    			value="<? if(isset($_POST[$field['id'].'_'.$lng])){ echo $_POST[$field['id'].'_'.$lng]; }elseif(count($records)){ echo implode(',', $records); }else{ echo $field['value'];}?>"
	    			name="<?=$field['id'].'_'.$lng?>"
	    			id="<?=$field['id'].'_'.$lng?>"
	    	...
	    	<? }else{ ?>
		        ...
		        <select
					name="<?=$field['id'].'_'.$lng?>[]"
					id="<?=$field['id'].'_'.$lng?>"
					class="form-control select2 w50"
					data-toggle="tooltip" data-placement="left" data-original-title="<?=$field['tooltip']?>"
					data-placeholder="<?=($field['placeholder']!="" ? $field['placeholder'] : _lng('choose'))?>"
					multiple
					<? if($field['do_not_edit']){?>disabled<? }?>>
					...
					foreach($this->values as $val){
						...
						if(is_numeric($field['max_levels']) && $field['max_levels']>0 && $val['level']>=$field['max_levels']) {
			        	...
			        	<option <?php if($option_disabled) echo 'disabled'; ?>
							value="<?=$val[$field['from_id']]?>" <? if(isset($_POST[$field['id'].'_'.$lng]) && in_array($val[$field['from_id']], $_POST[$field['id'].'_'.$lng])) echo "selected"; elseif(!isset($_POST['submit']) && in_array($val[$field['from_id']], $records)) echo "selected"?>>
			        		<?
			        		$add_to = '';
