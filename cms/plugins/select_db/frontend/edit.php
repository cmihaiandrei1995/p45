	...
	<div id="field_<?=$field_id.'_'.$lng?>"
		...
		<div class="col-md-6 col-lg-10 ">
		    ...
		    <? if($field['use_ajax']){?>
	    		...
	    		<input
	    			type="hidden"
	    			value="<? if(isset($_POST[$field['id'].'_'.$lng])){ echo $_POST[$field['id'].'_'.$lng]; }elseif(!isset($_POST['submit']) && $_record[$lng][$field['db_name']]!=""){ echo $_record[$lng][$field['db_name']]; }else{ echo $field['value'];}?>"
	    			name="<?=$field['id'].'_'.$lng?>"
	    			id="<?=$field['id'].'_'.$lng?>"
	    	...
	    	<? }else{ ?>
		    	...
		    	<select
					...
					?>
			        	<option <?php if($option_disabled) echo 'disabled'; ?> value="<?=$val[$field['from_id']]?>"
			        		<? if(isset($_POST[$field['id'].'_'.$lng]) && $_POST[$field['id'].'_'.$lng] == $val[$field['from_id']]) echo "selected"; elseif(!isset($_POST['submit']) && $_record[$lng][$field['db_name']] == $val[$field['from_id']]) echo "selected"?>>
			        		<?
			        		$add_to = '';
