...
if(!function_exists('select_db_format_records_with_parents')){
	function select_db_format_records_with_parents($fields, $field, $joins, $order_by, $parent_id, $level = 0){
		...
		foreach($records as $key => $val){
			...
			$children = select_db_format_records_with_parents($fields, $field, $joins, $order_by, $val[$field['from_id']], $val['level']);
			if(count($children)){
...
Usage:
		...
		'id_hotel' => array(												//name - for reference
			...
			'type' 					=> 'select_db',							//look for field types in cms/plugins
			...
			'use_ajax'				=> false,								//use ajax for search
