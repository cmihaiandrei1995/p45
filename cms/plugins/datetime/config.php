...
$this->view_settings = array(
	'is_viewable'		=> true,
	'view_callback'		=> 'convert_sql_datetime_to_js',
	'is_searchable' 	=> false,
	'is_filtrable'		=> false,
	'is_sortable'		=> true,
...
if(!function_exists('convert_sql_datetime_to_js')){
	function convert_sql_datetime_to_js($date, $fld = false){
		...
		if(is_array($fld)){
			...
			$to_format_time = $fld['js_time_format'];
		}else{
			$to_format = $_section['fields'][$fld]['js_format'];
			$to_format_time = $_section['fields'][$fld]['js_time_format'];
		}

		return ($date != "" ? date(str_replace(array('mm', 'dd', 'yy'), array('m', 'd', 'Y'), $to_format) . " " . str_replace(array('HH', 'mm', 'ss'), array('H', 'i', 's'), $to_format_time), strtotime($date)) : "");
	}
}
...
Usage:
		...
		'date' => array(
			...
			'db_type'				=> 'datetime',							//db name of the field
			'name' 					=> 'Data',								//name to appear on the left column
			'type' 					=> 'datetime',							//look for field types in cms/plugins

			'js_format'				=> 'dd.mm.yy ',							//js date format for showing
			'js_time_format'		=> 'HH:mm:ss',							//js time format for showing
			'db_format'				=> 'Y-m-d H:i:s',						//databse format to convert and save to
