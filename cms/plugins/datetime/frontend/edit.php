<? $i=1; foreach($field['lng'] as $lng){?>
	<? $_record[$lng][$field['db_name']] = ( $_record[$lng][$field['db_name']] != "" ? convert_sql_datetime_to_js($_record[$lng][$field['db_name']], $field) : "" ); ?>
	...
	<div id="field_<?=$field_id.'_'.$lng?>"
	    ...
	    <script type="text/javascript">
		    $(document).ready(function(){
		    	$("#<?=$field['id'].'_'.$lng?>").datetimepicker({
		    		firstDay: 1,
					//autoSize: true,
					dateFormat: '<?=($field['js_format'] != "" ? $field['js_format'] : 'dd.mm.yy')?>',
					timeFormat: '<?=($field['js_time_format'] != "" ? $field['js_time_format'] : 'HH:mm:ss')?>',
