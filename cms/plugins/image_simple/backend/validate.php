...
if(!($_action == "edit" && $field['do_not_edit'])){
	foreach($field['lng'] as $lng){
		...
		if($field['required'] && $_POST[$field['id'].'_'.$lng.'_id'] == ""){
			$_rules[$field['id'].'_'.$lng] .= '|requiredfile';
		}
		if($field['accepted_ext'] && $_POST[$field['id'].'_'.$lng.'_id'] == ""){
			$_rules[$field['id'].'_'.$lng] .= '|file-'.implode(",", $field['accepted_ext']);
		}
	}
}
