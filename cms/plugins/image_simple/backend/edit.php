...
if($field['folder'] != ""){
	...
	create_folders('images', date('Y', strtotime($_record['row']['created'])), date('n', strtotime($_record['row']['created'])), date('j', strtotime($_record['row']['created'])));
	$dirname = $_base_uploads_path.'images/'.date('Y', strtotime($_record['row']['created'])).'/'.date('n', strtotime($_record['row']['created'])).'/'.date('j', strtotime($_record['row']['created'])).'/';
}
		...
		foreach($field['lng'] as $lng){
			...
			if($_FILES[$image_field_name]["size"] != 0) {
				if($field['folder'] != ""){
					...
					foreach($field['sizes'] as $key => $field_val) {
						$img_path_other = $_base_uploads_path.'images/'.$folder.$key.'-'.$_record[$lng][$field['db_name']];
						if(file_exists($img_path_other)) {
							unlink($img_path_other);
						}
					}
