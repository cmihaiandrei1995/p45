...
{
  /*  */
  var $header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<?xml-stylesheet type=\"text/xsl\" href=\"lib/classes/google/sitemap.xsl\"?>\n<urlset xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd\" xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\" xmlns:image=\"http://www.google.com/schemas/sitemap-image/1.1\">";
  var $charset = "UTF-8";
  var $footer = "</urlset>\n";
  var $items = array();
    ...
    foreach($this->items as $item)
    {
    	...
    	if ( !empty( $item->lastmod ) )
	 	...
	 	// changefreq
	  	...
	  	//image
	  	if ( !empty( $item->image['loc'] ) ) {
			$map .= "\t\t<image:image>\n";
	  		
	  		if ( !empty( $item->image['loc'] ) )
	  		$map .= "\t\t\t<image:loc>".$item->image['loc']."</image:loc>\n";
			
			if ( !empty( $item->image['geo_location'] ) )
	  		$map .= "\t\t\t<image:geo_location>".$item->image['geo_location']."</image:geo_location>\n";
			
			if ( !empty( $item->image['title'] ) )
	  		$map .= "\t\t\t<image:title>".$item->image['title']."</image:title>\n";
			
			if ( !empty( $item->image['license'] ) )
	  		$map .= "\t\t\t<image:license>".$item->image['license']."</image:license>\n";
...
class google_sitemap_item
{
   ...
   */
  function google_sitemap_item( $loc, $lastmod = '', $changefreq = '', $priority = '', $image = array() )
  {
    $this->loc = $loc;
